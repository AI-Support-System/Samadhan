...
const contacts = [
{ id: 1, name: 'Emma Thompson', accountNumber: '**** 4582', avatar: 'https://public.readdy.ai/ai/img_res/a5424fe4024d9cf8f7b114d67197e4a4.jpg' },
{ id: 2, name: 'Michael Chen', accountNumber: '**** 7891', avatar: 'https://public.readdy.ai/ai/img_res/7ba67324bfd7939b648bb52bd8f48819.jpg' },
{ id: 3, name: 'Sarah Williams', accountNumber: '**** 3456', avatar: 'https://public.readdy.ai/ai/img_res/8825f7a7b8def59ed28a023bd38dfc98.jpg' }
];
const recentTransfers = [
{ id: 1, name: 'Emma Thompson', amount: -1250.00, currency: 'USD', purpose: 'Rent Payment', date: '2025-03-10' },
{ id: 2, name: 'Michael Chen', amount: -385.50, currency: 'EUR', purpose: 'Dinner Split', date: '2025-03-09' },
{ id: 3, name: 'Sarah Williams', amount: -750.00, currency: 'GBP', purpose: 'Project Payment', date: '2025-03-08' }
];
const purposes = ['Rent Payment', 'Dinner Split', 'Project Payment', 'Utilities', 'Services'];
...
<h2 className="text-2xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Money Transfer</h2>
<div className="flex mb-6 bg-[#2A2A2F] rounded-lg p-1">
<button
className={`flex-1 py-3 px-4 rounded-lg transition-all duration-300 !rounded-button whitespace-nowrap cursor-pointer ${activeTab === 'contacts' ? 'bg-gradient-to-r from-cyan-500 to-blue-600' : ''}`}
onClick={() => setActiveTab('contacts')}
>
From Contacts
</button>
<button
className={`flex-1 py-3 px-4 rounded-lg transition-all duration-300 !rounded-button whitespace-nowrap cursor-pointer ${activeTab === 'manual' ? 'bg-gradient-to-r from-cyan-500 to-blue-600' : ''}`}
...
<div key={contact.id} className="flex items-center gap-4 p-4 hover:bg-[#2A2A2F] rounded-lg cursor-pointer transition-colors mb-2">
<img src={contact.avatar} alt="" className="w-10 h-10 rounded-full object-cover"/>
<div>
<div className="font-medium">{contact.name}</div>
<div className="text-gray-400 text-sm">{contact.accountNumber}</div>
</div>
</div>
))}
</div>
...
>
<span>{currency}</span>
<i className={`fas fa-chevron-down transition-transform ${showCurrencyDropdown ? 'rotate-180' : ''}`}></i>
</button>
...
<i className={`fas fa-chevron-down transition-transform ${showPurposeDropdown ? 'rotate-180' : ''}`}></i>
</div>
{showPurposeDropdown && (
...
<h2 className="text-2xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Recent Transfers</h2>
...
{currencies.find(c => c.code === transfer.currency)?.symbol}{transfer.amount.toFixed(2)}
</div>
</div>
